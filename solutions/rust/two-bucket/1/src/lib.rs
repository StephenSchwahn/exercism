...
impl Bucket {
    ...
    pub fn pour_into(&mut self, other: &mut Bucket) {
...
pub fn solve(
    ...
    match (capacity_1, capacity_2) {
        ...
        (capacity_1, capacity_2) => {
            let mut this_bucket = Bucket {
                label: if *start_bucket == BucketLabel::One { BucketLabel::One } else { BucketLabel::Two },
                water_level: 0,
                capacity: if *start_bucket == BucketLabel::One { capacity_1 } else { capacity_2 }
            };

            let mut other_bucket = Bucket {
                label: if *start_bucket == BucketLabel::One { BucketLabel::Two } else { BucketLabel::One },
                water_level: 0,
                capacity: if *start_bucket == BucketLabel::One { capacity_2 } else { capacity_1 }
            };
