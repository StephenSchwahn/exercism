...
impl<T: Hash + Copy + PartialEq> CustomSet<T> {
    ...
    pub fn intersection(&self, other: &Self) -> Self {
        let common_elements: Vec<T> = self.items.iter()
            .flat_map(|vec| 
                vec.iter().filter(|item| other.contains(item)).copied()
            )
            .collect();
    ...
    pub fn union(&self, other: &Self) -> Self {
        Self::new(Vec::from_iter(
            self.items.iter().flat_map(|vec| vec.iter().copied()).chain(
                other.items.iter().flat_map(|vec| vec.iter().copied())
            )
        ).as_slice())
    }
}
